    ...
    const fetchSafetyData = async () => {
      try {
        ...
        const normalizedAlerts = alertList.map((item, index) => {
          ...
          let level = "INFO";
          if (
            msgContent.includes("경보") ||
            msgContent.includes("대피") ||
            msgContent.includes("위험")
          ) {
            level = "CRITICAL";
          } else if (
            msgContent.includes("주의보") ||
            msgContent.includes("자제")
          ) {
            level = "WARN";
          }
          ...
          return {
            ...
            title:
              item.dstSeNm || item.DST_SE_NM
                ? `${item.dstSeNm || item.DST_SE_NM} 상황 속보`
                : "재난안전 지침",
            content: msgContent,
        ...
        const warnCount = normalizedAlerts.filter(
          (a) => a.level === "WARN",
        ).length;
        const infoCount = normalizedAlerts.filter(
          (a) => a.level === "INFO",
        ).length;
    ...
    <div className="main-wrapper">
      ...
      <div className="safety-container">
        ...
        <div className="safety-header">
          <h2>
            <i className="fa-solid fa-triangle-exclamation"></i> 실시간 재난안전
            정보
          </h2>
          <p>
            국민재난안전포털의 데이터를 기반으로 한 실시간 속보 및
            행동요령입니다.
          </p>
        </div>
