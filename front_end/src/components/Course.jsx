    ...
    try {
      ...
      if (responseData && responseData.items && responseData.items.length > 0) {
        ...
        const mappedData = responseData.items.map((item, index) => ({
          id: index + 1 + (currentPage - 1) * numOfRows,
          title: item.parkNm,                           // 공원명
          address: item.lnmadr || "주소 정보 없음",       // 소재지지번주소
          // 보유 시설 정보들을 조합하여 설명란에 바인딩
          desc: [item.mvmFclty, item.cnvnncFclty, item.cltrFclty]
            .filter(Boolean)
    ...
    <div className="cr-page-global-container">
      ...
      <section className="cr-hero-illustration-banner">
        <div className="cr-banner-text-wrap">
          <h1>러닝 코스 추천</h1>
          <p>지도를 기반으로 실시간 제공되는 최적의 러닝 코스를 추천해드려요.</p>
        </div>
      </section>
      ...
      <main className="cr-main-split-layout">
        ...
        <aside className="cr-filter-sidebar-wrapper">
          ...
          <div className="cr-sidebar-form-group">
            <label>지역 검색</label>
            <select className="cr-form-combo-box" value={city} onChange={(e) => setCity(e.target.value)}>
        ...
        <section className="cr-center-cards-scroll-container">
          <div className="cr-list-top-meta-bar">
            <span className="cr-total-count-label">
              해당 구역 검색 결과 <strong className="count-num">{totalCount}</strong>개의 코스
            </span>

            <select className="cr-sort-dropdown-select" value={sortType} onChange={(e) => setSortType(e.target.value)}>
          ...
          {loading ? (
            <div style={{ textAlign: "center", padding: "80px 0", color: "#16A34A", fontWeight: "bold" }}>
              <i className="fa-solid fa-spinner fa-spin"></i> 데이터를 실시간으로 조회하는 중입니다...
            </div>
          ) : courses.length === 0 ? (
            ...
            <div className="cr-cards-vertical-stack">
              {courses.map((course) => (
                <div key={course.id} className="cr-item-row-card">
                  <div className="cr-card-thumbnail-area" style={{ backgroundColor: "#f1f5f9" }}>
                  ...
                  <div className="cr-card-main-details">
                    ...
                    <p className="cr-info-location-text">
                      <i className="fa-solid fa-location-dot"></i> {course.address}
                    </p>
                    <p className="cr-info-body-desc">{course.desc}</p>

                    <div className="cr-card-bottom-flex-bar" style={{ justifyContent: "flex-end" }}>
                      <button className="cr-view-detail-action-btn" onClick={() => navigate(`/course-detail/${course.id}?district=${district}`)}>
                        코스 상세 보기
                      </button>
                    </div>
                  </div>
          ...
          <div className="cr-pagination-nav-bar">
            ...
            {Array.from({ length: Math.ceil(totalCount / numOfRows) || 1 }, (_, i) => i + 1).slice(0, 5).map((num) => (
              <button 
                key={num} 
